...
def download_datasets(config=None):
    ...
    try:
        ...
        if config.cache_data:
            ...
            train_dataset.export(export_path=train_dataset_path, dataset_type=fo.types.FiftyOneDataset)
            val_dataset.export(export_path=val_dataset_path, dataset_type=fo.types.FiftyOneDataset)
...
def split_dataset(dataset, val_split=0.2):
    ...
    for sample in train_dataset:
        for detection in sample.ground_truth.detections:
            train_class_counts[detection.label] += 1
    
    for sample in val_dataset:
        for detection in sample.ground_truth.detections:
            val_class_counts[detection.label] += 1
