...
CLASSES = ['__background__', 'Coca Cola', 'Milk', 'Yogurt', 'Butter', 'cocolino', 'Wine glass', 'Drink']
...
def load_custom_dataset(custom_data_path, custom_labels_path, config=None):
    """
    Loads a custom COCO-formatted dataset.

    Args:
        custom_data_path: Path to the directory containing images
        custom_labels_path: Path to the COCO JSON annotations file
        config: Dataset configuration

    Returns:
        FiftyOne dataset containing the custom data
    """
    logger.info(f"Loading custom dataset from {custom_data_path} with annotations {custom_labels_path}")

    if not os.path.exists(custom_data_path):
        logger.error(f"Custom data path {custom_data_path} does not exist")
        return None

    if not os.path.exists(custom_labels_path):
        logger.error(f"Custom labels path {custom_labels_path} does not exist")
        return None

    try:
        # Create a unique dataset name
        custom_dataset_name = f"custom_grocery_{int(time.time())}"

        # Import the dataset using FiftyOne
        custom_dataset = fo.Dataset.from_dir(
            dataset_type=fo.types.COCODetectionDataset,
            data_path=custom_data_path,
            labels_path=custom_labels_path,
            include_id=True,  # Include COCO ids as metadata
            name=custom_dataset_name
        )

        logger.info(f"Loaded custom dataset with {len(custom_dataset)} samples")

        # Validate the dataset to ensure it has the expected format
        validate_custom_dataset(custom_dataset, config)

        return custom_dataset

    except Exception as e:
        logger.error(f"Error loading custom dataset: {str(e)}", exc_info=True)
        return None


def validate_custom_dataset(dataset, config=None):
    """
    Validates a custom dataset to ensure it has the expected format and classes.

    Args:
        dataset: FiftyOne dataset to validate
        config: Dataset configuration
    """
    if config is None:
        config = DatasetConfig()
    ...
    logger.info(f"Custom dataset class distribution: {class_counts}")

    if unexpected_classes:
        logger.warning(f"Found unexpected classes in custom dataset: {unexpected_classes}")
        logger.warning("These classes will be ignored during training")

    # Check if any expected classes are missing
    missing_classes = set(CLASSES[1:]) - set(class_counts.keys())  # Skip background
    if missing_classes:
        logger.warning(f"Custom dataset is missing expected classes: {missing_classes}")


def create_data_loaders_with_custom(config=None, input_size=224, batch_size=8, num_workers=4, 
                                    custom_data_path=None, custom_labels_path=None):
    """
    Create train, validation, and custom data loaders.
    ...
    Args:
        ...
        batch_size: Batch size for the data loaders
        num_workers: Number of workers for data loading
        custom_data_path: Path to custom dataset images
        custom_labels_path: Path to custom dataset annotations
        
    Returns:
        train_loader, val_loader, custom_loader: DataLoaders for training, validation, and custom dataset
    """
    if config is None:
        config = DatasetConfig()
    ...
    # Create standard Dataset objects
    train_dataset = GroceryDataset(
        root=config.train_images_dir,
        annFile=config.train_annotations_path,
        transform=get_transform(train=True, input_size=input_size),
    ...
    # Create custom dataset loader if paths are provided
    if custom_data_path and custom_labels_path:
        # Load custom dataset
        custom_dataset_fo = load_custom_dataset(custom_data_path, custom_labels_path, config)

        if custom_dataset_fo:
            # Create a compatible GroceryDataset
            custom_dataset = GroceryDataset(
                root=custom_data_path,
                annFile=custom_labels_path,
                transform=get_transform(train=False, input_size=input_size),
                augment=False
            )
            ...
            logger.info(f"Created custom data loader with {len(custom_dataset)} samples")

    logger.info(f"Created data loaders: train={len(train_dataset)} samples, val={len(val_dataset)} samples, "
               f"custom={len(custom_loader.dataset) if custom_loader else 0} samples")

    return train_loader, val_loader, custom_loader
...
def main():
    ...
    # Define paths to your custom dataset
    custom_data_path = Path(__file__).resolve().parent.parent / "own_database"
    custom_labels_path = Path(__file__).resolve().parent.parent / "own_database" / "annotations" / "own_coco.json"

    # Create data loaders including custom dataset
    train_loader, val_loader, custom_loader = create_data_loaders_with_custom(
        config=config,
        input_size=224,
        batch_size=8,
        num_workers=4,
    ...
    logger.info(f"Loaded train batch with {len(images)} images")

    # Test custom loader if available
    if custom_loader:
        custom_iter = iter(custom_loader)
        custom_images, custom_targets = next(custom_iter)
        logger.info(f"Loaded custom batch with {len(custom_images)} images")
        logger.info(f"Custom image shape: {custom_images[0].shape}")
        logger.info(f"Custom target keys: {custom_targets[0].keys()}")
        ...
        logger.info("Visualizing custom dataset samples:")
        visualize_dataset_samples(custom_dataset, num_samples=5, figsize=(15, 10))
